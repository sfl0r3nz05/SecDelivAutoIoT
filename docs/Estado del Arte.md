...
En este proyecto, se llevará a cabo una etapa de Integración Continua enfocada en la seguridad utilizando herramientas compatibles con frameworks como OWASP o NIST para la implementación de chequeos de seguridad de dispositivos IoT. Además, se aplicará GitOps para la Implementación Continua, acelerando la entrega de valor y evitar procesos manuales. El software se entregará en dispositivos IoT que soportan infraestructuras basadas en K3s, como Raspberry Pi y Beaglebones, y se monitorizará el cluster utilizando herramientas de monitorización como DataDog. La solución de automatización se implementará tanto en un entorno GitLab como en uno basado en Drones.
